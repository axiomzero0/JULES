...
| 12 | AlgebraicSimplification | IMPLEMENTED | Multi-pattern algebra with side conditions. NEW (2026-09-24): division/modulo by power-of-two constants, int scalar — unsigned x/2^k -> Shr, x%2^k -> And; signed via the round-toward-zero bias form (x + ((x>>a bits-1) & (2^k-1))) >>a k, mod composed as x - q<<k; negative-pow2 divisors negate the bias form; |d|==1 folds to identity/zero. Identities verified by jules/scripts/verify_magic_math.py; non-pow2 constant divisors stay for the isel magic-number rules (the per-arch layer). |
...
| 92 | Superoptimization | IMPLEMENTED | The search tier (2026-09-24, `src/superopt/` + `src/targets/x86_64/x64_super_isa.cpp`): a bounded, deterministic best-first search over post-88 MIR windows at -O3 only (kRowO3). The ENGINE is target-agnostic and contains ZERO instruction knowledge; the ISA table is the single source of x86-64 semantics as DATA (one row per IOp: operand shapes, variant axes, exact per-lane simulate closures incl. 32-bit zero-extension / partial slot writes / shift-count masking / undefined-flag poison, liveness effects incl. destructive-destination reads, latencies from DpIsel's cost classes). Per window: the live-out CONTRACT from real backward liveness over the label-region CFG; candidates built only from the original's own registers and constants; definedness rule (flags never available at entry; count-0 shift pass-through modeled); constant-sweep lanes seed window immediates so conditional code FIRES during the goal test (the anti-"cmp $K never matches on random garbage" policy that fixed five suite miscompiles); lazy-Dijkstra state dedup (cheapest program per state); goal-distance-first pop ordering; commit gate re-verifies on 7 fresh batches. Wins are search OUTPUTS: x*5 -> `leaq (x,x,4)`, x+x -> `leaq (,x,2)`, dead-window erasure. VERIFICATION LADDER (2026-09-25): Tier 1 in-search sampling -> Tier 2 fresh-batch gate -> Tier 3 concrete cross-probe (SMT encoder must reproduce the simulator bit-for-bit on batch-0 inputs, `JULES_SUPEROPT_SMT_SELFTEST=1`, suite passes under it) -> Tier 4 Z3 SMT equivalence PROOF per commit (`x64_super_smt.cpp`: shared symbolic inputs, exact faults, unconstrained undefined flags; Proven/Refuted/Unknown; Refuted rejects, Unknown degrades unless `JULES_SUPEROPT_SMT=2`). SEARCH-CORE THROUGHPUT restructured the same round: incremental XOR-fold fingerprints, write-set journal in-place sims (`JULES_SUPEROPT_AUDIT=1` proves the journal), once-per-window seeds, flat dedup — pops 0.23M->1.09M/s, constructions 0.64M->3.07M/s with identical exploration (4.75x wall); `JULES_SUPEROPT_TPUT=1` telemetry. Env: JULES_SUPEROPT / _STATS / _TRACE / _POPS / _FN_POPS / _CANDS / _WIN_CANDS / _STATES / _SMT / _SMT_SELFTEST / _SMT_TIMEOUT / _AUDIT / _TPUT. Locked by t_superopt (oracle diff + -O3 activity + kill switch + env switch + tier-4 proven>0 assertion), suite 395/395. |
